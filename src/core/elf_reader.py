"""plink v2.0 - ELF/DWARF 符号解析器

纯静态解析 ELF/AXF 文件，提取全局变量的名称、地址、类型、大小。
不启动任何子进程，不依赖硬件连接。使用 pyelftools 库。

替代 v1.0 的 gdb_mi_client.py + symbol_parser.py 组合。
"""
...
from .data_types import VariableInfo, VarType
...
class ElfSymbolReader:
            ...
            if symtab:
                for sym in symtab.iter_symbols():
                    if sym['st_info']['type'] == 'STT_OBJECT':
                        ...
                        size = sym['st_size']
                        name = sym.name
                        if name and addr >= _RAM_BASE and addr < _RAM_LIMIT and size > 0:
    ...
    def resolve(self, name: str) -> Optional[VariableInfo]:
        ...
        name 支持完整名（含命名空间）、短名、或 dot-notation（如 obj.member）。
        """
    ...
    def _walk_die(
        ...
        if tag == 'DW_TAG_namespace':
            ...
            for child in die.iter_children():
                self._walk_die(child, CU, namespace_stack + [ns_name], symtab_cache, enc_type_stack)
            ...
            if tname not in self._struct_defs:
                ...
                for child in die.iter_children():
                    if child.tag == 'DW_TAG_member':
                        ...
                        offset = loc_attr.value if loc_attr and isinstance(loc_attr.value, int) else 0
            ...
            self._process_variable_die(die, CU, namespace_stack, symtab_cache)
    ...
    def _process_variable_die(
        ...
        full_name = '::'.join(namespace_stack + [var_name]) if namespace_stack else var_name
    ...
    def _resolve_struct_members(self) -> None:
        ...
        递归解析嵌套结构体成员，使 _var_cache 包含所有层级的成员。
        """
