...
class MainWindow(QMainWindow):
    ...
    def _setup_ui(self) -> None:
        ...
        left_layout.addWidget(self._variable_panel, 1)
    ...
    def _on_flash(self) -> None:
        ...
        firmware_path = self._select_firmware_for_flash(config.elf_path)
        if not firmware_path:
            return
        if not Path(firmware_path).exists():
            QMessageBox.warning(self, "提示", "请选择有效的 ELF/AXF/HEX 固件文件")
            return


        fname = Path(firmware_path).name
        reply = QMessageBox.question(
            self, "确认烧录",
            f"即将以智能模式烧录固件到目标 MCU：\n\n{firmware_path}\n\n"
            "智能模式会跳过未变化的 Flash 页，并保留扇区内未覆盖的数据。\n"
            "烧录完成后将自动复位 MCU。\n"
            "是否继续？",
        ...
        progress = QProgressDialog("正在烧录固件...", None, 0, 100, self)
        ...
        try:
            ...
            self._backend.flash_firmware(firmware_path, progress_callback=progress_cb)
    ...
    def _select_firmware_for_flash(self, default_path: str) -> str:
        """选择用于烧录的固件文件。"""
        start_dir = str(Path(default_path).parent) if default_path else ""
        path, _ = QFileDialog.getOpenFileName(
            self,
            "选择要烧录的固件",
            default_path or start_dir,
            "固件文件 (*.elf *.axf *.hex *.ELF *.AXF *.HEX);;所有文件 (*)",
            "固件文件 (*.elf *.axf *.hex *.ELF *.AXF *.HEX)",
        )
        return path
