...
6. **管理监控变量**:
   - **选中变量**: 在监控列表中单击任意行即可选中该变量，点击列表空白区域可取消选中.
   - **删除变量**: 选中变量后，按下 `Delete` 键即可快速删除该变量; 也可右键选择 `[删除]` 进行删除.
   - **设置显示比例**: 监控表中每个通道可独立设置 `Scale` (乘法因子)和 `Offset` (加法偏移),以适配不同单位和范围的变量.
...
9. **修改变量值**: 在监控列表中双击变量的值列，可直接输入新值并写入 MCU 内存.
   - 支持十进制整数（如 `42`）、十六进制（如 `0x2A`）、浮点数（如 `3.14`）.
   - **浮点数写入整型变量**: 若目标变量为整数类型（如 `int`、`short`），输入浮点数将自动四舍五入为整数，状态栏会显示截断提示（如 "输入值 7.123 已四舍五入为 7"）.
   - **枚举变量**: 支持输入枚举名称（如 `STATE_IDLE`）或带值格式（如 `IDLE(0)`）.
   - 写入时会自动获取采样线程的写锁，避免与采样冲突；若采样线程繁忙导致写锁超时，状态栏会提示"写入超时".
10. **分析工具**:   
...
![波形分析窗](./pic/3.png)
11. **采样频率设置**: `[设置]` -> `[采样频率]` 可以设置数据采样的频率, 以适应不同应用场景的需求.
12. **CSV数据导出**: 
